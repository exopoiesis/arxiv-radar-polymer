#!/usr/bin/env python3
"""Apply polymer-context filter to existing corpus locally.
...
paper's abstract is scanned for polymer-relevant keywords; non-matching
...
import data_io
from data_io import is_polymer_paper  # noqa: F401  (re-export for tests/CLI)
...
def run(out_dir):
    ...
    for month, papers in sorted(by_month.items()):
        ...
        for pid, rec in papers.items():
            if is_polymer_paper(rec.get("abstract", "")):
                kept[pid] = rec
