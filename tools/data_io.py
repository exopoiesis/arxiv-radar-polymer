...
# -- Polymer filter ------------------------------------------------------------
# Word-boundary regex of polymer-relevant terms. Stems (polymer, macromolecul)
# catch inflections; short abbreviations are bound to whole-word
...
POLYMER_PATTERN = re.compile(
    r"\b(?:"
    r"polymer\w*|polymeric|polymers?|"
    r"copolymer\w*|block\s+copolymer\w*|"
    r"monomer\w*|oligomer\w*|macromolecul\w*|macromer\w*|"
    r"polymerization|polymerisation|polymeriz\w*|polymeris\w*|"
    r"bigsmiles|selfies|smiles|qspr|"
    r"glass\s+transition|glass-transition|tg\s+prediction|"
    r"flory[-\s]huggins|viscoelastic\w*|rheolog\w*|"
    r"elastomer\w*|thermoplastic\w*|thermoset\w*|resin\w*|"
    r"hydrogel\w*|membrane\w*|fiber\w*|fibre\w*|"
    r"nanocomposite\w*|composite\w*|filler\w*|"
    r"crystallinity|crystalline\s+polymer\w*|amorphous\s+polymer\w*|"
    r"phase\s+separation|self[-\s]assembl\w*|morpholog\w*|microstructure\w*|"
    r"coarse[-\s]grained|cgmd|soft\s+matter|"
    r"polymer\s+electrolyte\w*|organic\s+photovoltaic\w*|"
    r"plastic\s+recycling|biodegradable\s+polymer\w*|"
    r"additive\s+manufacturing|3d\s+printing\s+polymer\w*|"
    r"extrusion|injection\s+molding|injection\s+moulding"
    r")\b",
    re.IGNORECASE,
)


def is_polymer_paper(abstract):
    """True if abstract contains at least one polymer-context term."""
    if not abstract:
        return False
    return bool(POLYMER_PATTERN.search(abstract))


# Backwards-compatible alias for older helper scripts copied from chemistry.
# New polymer code should import is_polymer_paper.
is_chemistry_paper = is_polymer_paper
    ...
    Each filter list becomes an OR-joined query. Per-filter rules:
      ...
      - Prefix 'RAW:' → expression passed through, wrapped in parens.
        Lets you write compound queries:
            RAW:"Large Language Models" AND (polymer OR macromolecular)
