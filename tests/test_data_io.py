...
def test_is_polymer_paper_positive():
    """Real polymer terms trigger the filter."""
    import data_io
    assert data_io.is_polymer_paper("Glass transition temperature prediction for polymers.")
    assert data_io.is_polymer_paper("Coarse-grained simulation of block copolymers.")
    assert data_io.is_polymer_paper("Radical polymerization of styrene.")


def test_is_polymer_paper_negative():
    """Non-polymer text doesn't match."""
    import data_io
    assert not data_io.is_polymer_paper("LLM for code generation.")
    assert not data_io.is_polymer_paper("Robot navigation in unknown environments.")
    assert not data_io.is_polymer_paper("")
...
def test_load_keyword_queries_raw_passthrough(tmp_path):
    ...
    assert '"Polymer LLMs"' in q
    assert '("Large Language Models" AND (polymer OR macromolecular))' in q
    assert " OR " in q
